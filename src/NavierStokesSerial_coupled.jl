function run_test_serial(mesh_file::String,force_file::String,Δt,tf,Δtout)
  ...
  res(t,(u,p,η),(v,q,κ)) =
    ∫( (sym_lapl∘(ε(u),ε(v))) )dΩ_f +
    ∫( (skew_conv∘(u,u,v,∇(u),∇(v))) )dΩ_f +
    ∫( (div_term∘((∇⋅u),q)) )dΩ_f -
    ∫( (div_term∘((∇⋅v),p)) )dΩ_f +
    ∫( (τₘ∘(u,h,h2)) * ((𝒫ᵤ∘(u,∇(u),∇(p),η))⋅((𝒞ᵤ∘(u,∇(v)))+∇(q)-κ)) )dΩ_f +
    ∫( (τc∘(u,h,h2)) * ((∇⋅u)*(∇⋅v)) )dΩ_f+
    # conv(u,u,v,dΩ_f) +
    # lap(ν_f,u,v,dΩ_f) -
    # div(v,p,dΩ_f) +
  ...
  jac(t,(u,p,η),(du,dp,dη),(v,q,κ)) =
    ∫( (sym_lapl∘(ε(du),ε(v)))  )dΩ_f +
    ∫( 0.5*((𝒞ᵤ∘(du,∇(u)))⋅v + (𝒞ᵤ∘(u,∇(du)))⋅v - (𝒞ᵤ∘(du,∇(v)))⋅u - (𝒞ᵤ∘(u,∇(v)))⋅du) )dΩ_f +
    ∫( (div_term∘((∇⋅du),q)) )dΩ_f -
    ∫( (div_term∘((∇⋅v),dp)) )dΩ_f +
    ∫( (τc∘(u,h,h2)) * ((∇⋅du)*(∇⋅v)) )dΩ_f +
